...
function registerSimulationRoutes(app, simulationQueue, repository) {
  app.get('/clear-simulations', (req, res) => {
    ...
    res.json(ok({ message: 'Simulations queue cleared', SIMULATIONS_QUEUE: simulationQueue.all() }));
  });
  ...
  app.get('/add-in-simulation-queue', (req, res) => {
    ...
    res.json(ok({ message: 'Simulation added to queue', SIMULATIONS_QUEUE: simulationQueue.all() }));
  });
